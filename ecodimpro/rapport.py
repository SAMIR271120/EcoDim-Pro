...
def generer_rapport_pdf(resultats: dict, client_info: dict, logo_path: str = None, output_path: str = "rapport.pdf") -> str:
    ...
    # ==========================================
    # PAGE 5 : RÉSULTATS DE PRODUCTION & CÂBLAGE
    # ==========================================
    # Section 4 : Production PV (Puce ambre)
    _s4_titre = Table(
        [[PuceCouleur(COULEURS["ambre"], taille=9, decalage_y=3),
          Paragraph("4. Analyse de Production Solaire &amp; Injection Reseau", _titre_h1)]],
        colWidths=[16, 471], hAlign="LEFT"
    )
    _s4_titre.setStyle(TableStyle([
        ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
        ('LEFTPADDING', (0,0), (-1,-1), 0), ('RIGHTPADDING', (0,0), (-1,-1), 0),
        ('TOPPADDING', (0,0), (-1,-1), 0), ('BOTTOMPADDING', (0,0), (-1,-1), 0),
    ]))
    story.append(_s4_titre)
    story.append(Spacer(1, 4))
    ...
    if has_bat:
        contexte_bat = (
            ...
            "le surplus de production d'electricite est injecte sur le reseau national."
        )
    else:
        contexte_bat = (
            "Apres alimentation en direct des appareils de la maison, le surplus de production d'electricite "
            "non consomme instantanement est injecte directement sur le reseau national."
        )

    story.append(Paragraph(
        f"Production photovoltaique annuelle estimee : <b>{prod_pv_an:,.0f} kWh/an</b>. "
        f"{contexte_bat} Cette injection permet de degager un revenu annuel complementaire.",
        body_style
    ))
    story.append(Spacer(1, 5))
    ...
    if os.path.exists(path_prod):
        try:
            story.append(Image(path_prod, width=15 * cm, height=5.0 * cm))
    ...
    if os.path.exists(path_vs):
        try:
            story.append(Image(path_vs, width=15 * cm, height=5.0 * cm))
            story.append(Spacer(1, 6))
        except Exception:
            pass
    ...
    if os.path.exists(path_surplus):
        try:
            story.append(Image(path_surplus, width=15 * cm, height=5.0 * cm))
            story.append(Spacer(1, 6))
        except Exception:
            pass
    ...
    # Placer la section cablage dans sa propre page pour eviter toute coupure
    story.append(PageBreak())

    # Section 5 : Cablage (Puce ambre)
    _s5_titre = Table(
        [[PuceCouleur(COULEURS["ambre"], taille=9, decalage_y=3),
          Paragraph("5. Cablage et Conformite Electrique (Norme NF C 15-100)", _titre_h1)]],
        colWidths=[16, 471], hAlign="LEFT"
    )
    ...
    # ==========================================
    # PAGE 6 : BUDGET, RENTABILITÉ & HYPOTHÈSES
    # ==========================================
    # Section 6 : Analyse Economique (Puce bleu_gris)
    _s6_titre = Table(
        [[PuceCouleur(COULEURS["bleu_gris"], taille=9, decalage_y=3),
          Paragraph("6. Analyse Economique &amp; ROI", _titre_h1)]],
        colWidths=[16, 471], hAlign="LEFT"
    )
    _s6_titre.setStyle(TableStyle([
        ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
        ('LEFTPADDING', (0,0), (-1,-1), 0), ('RIGHTPADDING', (0,0), (-1,-1), 0),
        ('TOPPADDING', (0,0), (-1,-1), 0), ('BOTTOMPADDING', (0,0), (-1,-1), 0),
    ]))
    story.append(_s6_titre)
    story.append(Spacer(1, 4))
    ...
    if os.path.exists(path_cash):
        try:
            story.append(Image(path_cash, width=15 * cm, height=6.8 * cm))
            story.append(Spacer(1, 15))
        except Exception:
            pass

    # Section 7 : Hypotheses & Limites (Puce bleu_gris)
    _s7_titre = Table(
        [[PuceCouleur(COULEURS["bleu_gris"], taille=9, decalage_y=3),
          Paragraph("7. Hypotheses, Methodologie et Limites", _titre_h1)]],
        colWidths=[16, 471], hAlign="LEFT"
    )
    _s7_titre.setStyle(TableStyle([
        ('VALIGN', (0,0), (-1,-1), 'MIDDLE'),
        ('LEFTPADDING', (0,0), (-1,-1), 0), ('RIGHTPADDING', (0,0), (-1,-1), 0),
        ('TOPPADDING', (0,0), (-1,-1), 0), ('BOTTOMPADDING', (0,0), (-1,-1), 0),
    ]))
    story.append(_s7_titre)
    story.append(Spacer(1, 4))
    story.append(Paragraph(
